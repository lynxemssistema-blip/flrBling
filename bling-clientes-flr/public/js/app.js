...
async function showDashboard() {
  ...
  // Se for superadmin, carrega contagem de usuários pendentes
  if (state.currentUser && state.currentUser.role === 'superadmin') {
    elements.superadminNavSection.style.display = 'block';
    elements.btnOpenAuthModal.style.display = 'flex';
    loadUsersList();
    ...
    elements.superadminNavSection.style.display = 'none';
    elements.btnOpenAuthModal.style.display = 'none';
  }
...
async function loadAppConfig() {
  try {
    ...
    if (state.appConfig && state.appConfig.authorizeUrl) {
...
async function checkBlingAuthStatus() {
  try {
    ...
    if (state.isBlingAuthenticated) {
      ...
      elements.apiStatusText.textContent = 'Bling Conectado';
      ...
      elements.btnLogoutToken.style.display = 'inline-block';
    } else {
      elements.apiStatusBadge.className = 'api-status-badge';
      elements.apiStatusText.textContent = 'Bling Não Conectado';
      ...
      elements.btnLogoutToken.style.display = 'none';
    }
  } catch (err) {
    elements.apiStatusBadge.className = 'api-status-badge error';
    elements.apiStatusText.textContent = 'Erro Bling';
  }
}
